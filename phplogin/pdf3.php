...
if (isset($_POST['rollnumber'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        $imagePathLeft = realpath('left1.png'); // Use the realpath function to get the absolute path
        $imagePathRight = realpath('');

        // Debugging image path
        if (!$imagePathLeft || !$imagePathRight) {
            die("Image file not found. Left: $imagePathLeft, Right: $imagePathRight");
        }

        // Add the first image to the top left
        if (file_exists($imagePathLeft)) {
            $pdf->Image($imagePathLeft, 10, 27, 25, 28);
        } else {
            echo "Left image not found: " . $imagePathLeft;
        }

        // Add the second image to the top right
        if (file_exists($imagePathRight)) {
            $pdf->Image($imagePathRight, $pdf->getPageWidth() - 36, 27, 20, 20);
        } else {
            echo "Right image not found: " . $imagePathRight;
        }
       ...
       &nbsp; &nbsp; &nbsp; &nbsp;<table>
        ...
        $html .= "</table>";
        $html.="<p> &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;This certificate is issued to enable $m to apply for <strong>$reason</strong> on $g own request.</p>";
