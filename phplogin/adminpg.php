    ...
    if (isset($_POST['reject'])) {
        ...
        if ($result_email && $result_email->num_rows > 0) {
            ...
            if (!empty($studentmail)) {
                ...
                $sql_rejection = "INSERT INTO rejectionpg (rollnumber, course, year, reason) VALUES ('$rollnumber', '$course', '$year', '$reason')";
                if ($conn->query($sql_rejection) === TRUE) {
                    ...
                    try {
                        ...
                        echo '<script>setTimeout(function(){ window.location.href = "adminpg.php"; }, 0.5);</script>';
                    } catch (Exception $e) {
                        // Show error message
                        echo '<script>Toastify({
                    ...
                    if ($conn->query($sql_delete_entry) === TRUE) {
                        ...
                        $sql_delete_pgtrack = "DELETE FROM pgtrack WHERE rollnumber = '$rollnumber'";
                        $conn->query($sql_delete_pgtrack);
                    } else {
                        ...
                        <td>
                           <form id="rejectForm_<?php echo $row['rollnumber']; ?>" action="" method="post"
                            ...
                            <div id="buttonContainer_<?php echo $row['rollnumber']; ?>"  style="display: inline-block;">
                                <form action="pgbonafide.php" method="post" style="display:inline-block;">
                                    <input type="hidden" name="rollnumber" value="<?php echo $row['rollnumber']; ?>">
                                    <button type="submit" class="btn btn-primary download-btn">Generate PDF</button>
                                </form>
                                <form action="feespg.php" method="post"  style="display: inline-block;">
                                    <input type="hidden" name="rollnumber" value="<?php echo $row['rollnumber']; ?>">
