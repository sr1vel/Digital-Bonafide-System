...
try {
    ...
    // Fetch data from all tables and combine them
    $stmtUganalysis = $pdo->prepare("SELECT 'UG' as type, course, year, reason, Date, rollnumber FROM uganalysis");
    $stmtUganalysis->execute();
    $uganData = $stmtUganalysis->fetchAll(PDO::FETCH_ASSOC);

    $stmtPganalysis = $pdo->prepare("SELECT 'PG' as type, course, year, reason, Date, rollnumber FROM pganalysis");
    $stmtPganalysis->execute();
    $pgData = $stmtPganalysis->fetchAll(PDO::FETCH_ASSOC);

    $stmtPassedanalysis = $pdo->prepare("SELECT 'PASSED' as type, course, NULL as year, reason, Date, rollnumber FROM passedanalysis");
    $stmtPassedanalysis->execute();
    $passedData = $stmtPassedanalysis->fetchAll(PDO::FETCH_ASSOC);

    // Combine all data into a single array
    $combinedData = array_merge($uganData, $pgData, $passedData);
    ...
    // Fetch data from all approval tables and combine them
    $stmtApproveCurrent = $pdo->prepare("SELECT course, year, reason, Date, rollnumber FROM approvecurrent");
    $stmtApproveCurrent->execute();
    $approveCurrentData = $stmtApproveCurrent->fetchAll(PDO::FETCH_ASSOC);

    $stmtApprovePg = $pdo->prepare("SELECT course, year, reason, Date, rollnumber FROM approvepg");
    $stmtApprovePg->execute();
    $approvePgData = $stmtApprovePg->fetchAll(PDO::FETCH_ASSOC);

    $stmtApprovePassed = $pdo->prepare("SELECT course, NULL as year, reason, Date, rollnumber FROM approvepassed");
    $stmtApprovePassed->execute();
    $approvePassedData = $stmtApprovePassed->fetchAll(PDO::FETCH_ASSOC);

    // Combine approval data
    $pgData = array_merge($approveCurrentData, $approvePgData, $approvePassedData);

    // Fetch data from all rejection tables and combine them
    $stmtReject = $pdo->prepare("SELECT course, reason, rollnumber, Date FROM rejection");
    $stmtReject->execute();
    $rejectData = $stmtReject->fetchAll(PDO::FETCH_ASSOC);

    $stmtRejectPg = $pdo->prepare("SELECT course, reason, rollnumber, Date FROM rejectionpg");
    $stmtRejectPg->execute();
    $rejectPgData = $stmtRejectPg->fetchAll(PDO::FETCH_ASSOC);

    $stmtRejectPassed = $pdo->prepare("SELECT course, reason, rollnumber, Date FROM rejectionpassed");
    $stmtRejectPassed->execute();
    $rejectPassedData = $stmtRejectPassed->fetchAll(PDO::FETCH_ASSOC);

    // Combine rejection data
    $passedData = array_merge($rejectData, $rejectPgData, $rejectPassedData);
...
<head>
    ...
    <style>
        ...
        .dropdown {
    position: relative;
    display: inline-block; /* Changed from list-item to inline-block for better positioning */
    margin-left: 1200px; /* This will push the dropdown to the right, as per your layout */
}
...
.dropdown:hover .dropdown-content {
    display: flex; /* Ensures the dropdown content displays in a flex container */
    flex-direction: column; /* Stacks items vertically */
}

.dropdown:hover .dropbtn {
    background-color: #3e8e41; /* Changes button color on hover */
}
            ...
            <tbody>
                <?php foreach ($pgData as $row): ?>
                    <tr>
                        <td><?php echo htmlspecialchars($row['Date']); ?></td>
                        <td><?php echo htmlspecialchars($row['course']); ?></td>
                        <td><?php echo isset($row['year']) ? htmlspecialchars($row['year']) : 'PASSED'; ?></td>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            function exportToPDF() {
                ...
                addTableToPDF('Combined Analysis Data', ['Type', 'Date', 'Course', 'Year', 'Reason', 'Roll Number'], combinedData.map(row => ({
                    Type: row.type,
                    Date: row.Date,
                    Course: row.course,
                    Year: row.year || 'PASSED',
                    Reason: row.reason,
