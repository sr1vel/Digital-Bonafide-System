...
try {
    ...
    $stmtPassed = $pdo->prepare("SELECT course, reason, rollnumber, Date FROM passedanalysis");
    $stmtPassed->execute();
    $passedData = $stmtPassed->fetchAll(PDO::FETCH_ASSOC);

    // Fetch all data from pganalysis
    $stmtPg = $pdo->prepare("SELECT course, year, reason, Date, rollnumber FROM approvepg");
    $stmtPg->execute();
    ...
    $stmtPassedrej = $pdo->prepare("SELECT course, reason, rollnumber, Date FROM rejectionpg");
    $stmtPassedrej->execute();
    $passedrejData = $stmtPassed->fetchAll(PDO::FETCH_ASSOC);
    ...
    $totalStudentsApplied = $totalPassed;
