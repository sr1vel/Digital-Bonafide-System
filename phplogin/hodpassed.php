...
if (isset($_SESSION['username'])) {
    ...
    $sql = "SELECT * FROM passed WHERE hmail = '$username'";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_verification'])) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        if (isset($_POST["HeadOfDepartment_$rollnumber"])) {
            ...
            $insert_sql = "INSERT INTO passedtrack (rollnumber, hod) VALUES ('$rollnumber', 'verified') ON DUPLICATE KEY UPDATE hod = 'verified'";
            $conn->query($insert_sql);

            // Delete data from passed table
            $delete_sql = "DELETE FROM passed WHERE rollnumber = '$rollnumber'";
                    ...
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            ...
                            <tr>
                                ...
                                <td><a href="<?php echo htmlspecialchars($row['IDProof']); ?>" class="btn btn-primary download-btn" download>Download</a></td>
                                ...
                                <td>
                                    <div class="checkbox-container">
                                        ...
                                        <input type="checkbox" name="HeadOfDepartment_<?php echo $row['rollnumber']; ?>"
                                            value="HeadOfDepartment" <?php echo (isCheckboxFilled($row['rollnumber'], 'hod')) ? ' checked disabled' : ''; ?>>
...
<?php
// Function to check if checkbox is filled
